...
# `spec-plan-build unblock 031 --commit` printed nothing at all: not what it
...
# case it is.
RSpec.describe SpecPlanBuild::CLI::Unblock, :tree do
  ...
  def with_executor(result: [true, "completed"], &edit)
    executor = instance_double(SpecPlanBuild::Executor)
    allow(executor).to receive(:call) do |_agent, subject, **|
      edit&.call(subject)
      result
    end
    allow(SpecPlanBuild::Executor).to receive(:new).and_return(executor)
  end
  ...
  def fold(subject, *numbers)
    ...
    remaining = kept.join
    remaining.match?(SpecPlanBuild::OPEN_BLOCK) ? File.write(path, remaining) : File.delete(path)
  end
end

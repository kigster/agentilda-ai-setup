...
# build.
RSpec.describe SpecPlanBuild::Diagram do
  ...
  describe "#render" do
    ...
    it "draws exactly the edges the machine allows — no more, no fewer" do
      actual = SpecPlanBuild::StateMachine.inbound.flat_map { |to, froms| froms.map { |from| [from, to] } }
    ...
    it "walks the main spine from New through Deployed, in order, on one line" do
      ...
      while key
        chain << key
        key = SpecPlanBuild::StateMachine::SPINE[key]
      end

      spine_line = rendered.lines.find { |l| l.include?("New") && l.include?("Deployed") }

      expect(spine_line).not_to be_nil
      positions = chain.map { |k| spine_line.index(SpecPlanBuild::STATUS_BY_KEY.fetch(k).label) }
    ...
    it "lists every terminal state — the ones nothing follows" do
      terminal = SpecPlanBuild::STATUSES.select(&:terminal?)
    ...
    it "flags the look-alike families the state machine defines" do
      SpecPlanBuild::StateMachine::FAMILIES.each do |members|
        members.each { |key| expect(rendered).to include(SpecPlanBuild::STATUS_BY_KEY.fetch(key).label) }
      end
    end

    it "points back at the state machine as the source" do
      expect(rendered).to include("lib/spec_plan_build/state_machine.rb")
    end
  ...
  describe "drift" do
    ...
    it "draws exactly as many terminal states as StateMachine reports" do
      ...
      expect(drawn.size).to eq(SpecPlanBuild::STATUSES.count(&:terminal?))
    end
    ...
    it "draws exactly as many spine rejoins as StateMachine::SPINE has off-chain entries" do
      ...
      off_chain = SpecPlanBuild::StateMachine::SPINE.except(*chain)
  ...
  def label_in(line) = SpecPlanBuild::STATUSES.find { |s| line.include?(s.label) }&.key

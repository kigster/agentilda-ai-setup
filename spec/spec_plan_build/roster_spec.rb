...
# `spec-plan-build agents`. Everything here is read from the definition files,
...
# prevent.
RSpec.describe SpecPlanBuild::Roster do
  subject(:roster) { described_class.new(agents: SpecPlanBuild::Agents.new(dir: dir)) }
  ...
  describe "#list" do
    ...
    it "says so plainly when there are no definitions to list" do
      Dir.mktmpdir("empty") do |empty|
        expect(described_class.new(agents: SpecPlanBuild::Agents.new(dir: empty)).list)
  ...
  describe "#describe" do
    ...
    it "names the agents it does know when asked for one it does not" do
      expect { roster.describe("nope") }
        .to raise_error(SpecPlanBuild::Error, /No agent called nope.*luke-implementer/m)
    end
  end
end

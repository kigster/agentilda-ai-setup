...
# Backs the plain (no `--prs`) path of `spec-plan-build create`. `synthesize`
# already has its own coverage through {SpecPlanBuild::Executor}; these
# examples are the other half of the fork in `CLI::Create#created` — the one
# for a feature that does not exist yet.
RSpec.describe SpecPlanBuild::Brief, :tree do
  ...
  describe "#invocation" do
    ...
    context "when the project keeps a backlog" do
      before do
        FileUtils.mkdir_p(File.join(root, SpecPlanBuild::PLANS_DIR))

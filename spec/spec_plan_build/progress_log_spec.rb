...
RSpec.describe SpecPlanBuild::ProgressLog do
  ...
  describe ".render" do
    ...
    it "holds every column open when a round header has nothing to put in them" do
      ...
      aggregate_failures do
        ...
        expect(SpecPlanBuild::UI.display_width(prefix_of(header)))
          .to eq(SpecPlanBuild::UI.display_width(prefix_of(full)))
        expect(SpecPlanBuild::UI.display_width(prefix_of(header))).to eq(described_class::LINE_WIDTH)
      end
    end
    ...
    it "cuts a name too long for its column rather than pushing the columns out" do
      ...
      aggregate_failures do
        ...
        expect(SpecPlanBuild::UI.display_width(prefix_of(line))).to eq(described_class::LINE_WIDTH)
      end
    end
    ...
    it "lands the separator after an emoji status in the same column as one without" do
      ...
      aggregate_failures do
        expect(SpecPlanBuild::UI.display_width(prefix_of(emoji))).to eq(described_class::LINE_WIDTH)
        expect(SpecPlanBuild::UI.display_width(prefix_of(plain))).to eq(described_class::LINE_WIDTH)
        expect(emoji.index("|", emoji.index("💩"))).not_to eq(plain.index("|", plain.index("Planned")))
        expect(SpecPlanBuild::UI.display_width(emoji[0...emoji.index("|", emoji.index("💩"))]))
          .to eq(SpecPlanBuild::UI.display_width(plain[0...plain.index("|", plain.index("Planned"))]))
      end
    end
  ...
  describe "the column widths" do
    it "fits the widest state the state machine defines, emoji included" do
      widest = SpecPlanBuild::STATUSES.map { |status| SpecPlanBuild::UI.display_width(status.to_s) }.max

      aggregate_failures do
        expect(described_class::STATUS_WIDTH).to eq(widest)
        expect(described_class::STATUS_WIDTH).to be >= SpecPlanBuild::UI.display_width("💩 Scrapped by Review")
      end
    end

    it "fits the longest specialist name on disk" do
      longest = SpecPlanBuild::Agents.new.all.map { |agent| agent.name.length }.max

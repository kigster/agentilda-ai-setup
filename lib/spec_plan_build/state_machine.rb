...
module SpecPlanBuild
  ...
  class StateMachine
    ...
    aasm do
      ...
      SpecPlanBuild::STATUSES.each { |status| state status.key }
      ...
      event :discard, guard: :justified? do
        transitions from: SpecPlanBuild::STATUSES.map(&:key) - %i[discarded], to: :discarded
      end
    ...
    # @return [SpecPlanBuild::Status] the state now occupied
    # @raise [SpecPlanBuild::StateMachine::Refused]

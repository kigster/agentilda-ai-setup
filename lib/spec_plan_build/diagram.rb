...
module SpecPlanBuild
  # `spec-plan-build states` — the state machine as a terminal picture.
  #
  ...
  class Diagram
    ...
    def heading
      paint("Spec → Plan → Build — the state machine", :bold) + "\n" +
        paint("(derived from lib/spec_plan_build/state_machine.rb — `spec-plan-build docs` for the Markdown form)",
          :bright_black)
    end

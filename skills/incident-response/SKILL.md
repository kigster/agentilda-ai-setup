...
1. **Assess severity.** Use the rubric. Open the appropriate response channel.
1. **Assign roles.** IC, comms, ops at minimum.
1. **Communicate.** Initial status update. Internal channel active.
1. **Investigate.** Logs, metrics, recent changes. The four most common causes: a recent deploy, a configuration change, a third-party dependency change, a load spike.
1. **Mitigate.** Stop the bleeding. Don't wait for full root cause.
1. **Verify mitigation.** Don't trust dashboards alone; test the user flow.
1. **Communicate resolution.** Internal and external.
1. **Close incident.** Final timeline noted. Action items tracked.
1. **Schedule AAR.** Within 1 to 2 weeks.

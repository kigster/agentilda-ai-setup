...
1. **Event catalog.** Every event, with properties, when fired, why tracked.
1. **User properties.** Persistent attributes (plan, signup_date, role).
1. **Page taxonomy.** Page categories, page properties.
1. **Naming conventions.** Snake_case, verb_noun, etc.
1. **Implementation notes.** Client-side vs server-side, SDK details, sampling.
1. **Privacy and compliance.** PII rules, consent handling, data retention.
1. **Governance.** Who can add events, review process, change log.
...
1. **Define the north star.** One metric. Tested against the criteria above.
1. **Build the KPI hierarchy.** Acquisition, activation, engagement, retention, monetization.
1. **Audit existing tracking.** What's there? What's broken? What's missing?
1. **Design the event taxonomy.** Cover the user journey. Document everything.
1. **Implement with care.** Test each event. Verify properties. Catch issues in staging.
1. **Build dashboards.** One per audience. Lead with action.
1. **Establish review cadence.** Weekly business review, monthly KPI review, quarterly strategy review.
1. **Govern.** Who adds events, who reviews, how changes propagate.

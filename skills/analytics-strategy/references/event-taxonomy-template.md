...
✓ `user_signed_up` ✓ `product_added_to_cart` ✓ `article_50_percent_scrolled` ✗ `signupCompleted` ✗ `addProduct` ✗ `Article 50% Scrolled`

### Properties: camelCase or snake_case (pick one and stay)

snake_case is more common in SQL contexts (data warehouses). camelCase is more common in JavaScript/TypeScript contexts. The choice matters less than consistency.

✓ `user_id`, `signup_method`, `email_type` (consistent snake_case) ✓ `userId`, `signupMethod`, `emailType` (consistent camelCase) ✗ `userId`, `signup_method`, `EmailType` (mixed)

### Property values: standardize where possible

Don't allow free-text where an enum should exist.

✓ `signup_method: "email" | "google" | "github" | "sso"` ✗ `signup_method: "email", "Email", "EMAIL", "via email", "with email address"`

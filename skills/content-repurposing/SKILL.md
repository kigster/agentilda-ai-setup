...
1. **Source-piece selection.** Strong sources have travelable arguments and standalone sections.
1. **Format adaptation patterns chosen.** Match source-piece characteristics to derivative formats.
1. **Per-format constraints respected.** Each medium's conventions, length norms, and reader expectations.
1. **Voice consistency across formats.** Brand POV stays; sentence structure adapts.
1. **Sequencing and cadence planned.** Source-first vs simultaneous; concentrated vs distributed.
1. **Cross-promotion across derivatives.** Linking, attribution, co-promotion.
1. **AI-search extraction.** FAQ, snippet, statistic, entity extractions designed deliberately.
1. **Cannibalization avoided.** Derivatives complement the source; do not compete for the same query.
1. **Engagement-per-format measured.** Derivatives evaluated on each medium's metrics.
1. **AI-assisted repurposing with voice discipline.** Voice prompts, per-derivative review, slop prevention.
1. **Capacity allocation explicit.** Repurposing is real work; budgeted, not done in the cracks.

...
1. **Run the snippet.** Paste the appropriate console snippet, review output.
1. **Note failures.** Each failure either gets fixed before ship or filed as a known issue.
1. **For Standard tier**, add: visual review at 375px, 768px, 1440px. Test the primary user flow.
1. **For Full tier**, add: cross-browser testing, Lighthouse audit, schema validation, security headers, 404 handling.
1. **Document.** Use the template in [`references/qa-report-template.md`](references/qa-report-template.md) for full audits.

...
1. **Fill in values** per the brand identity.
1. **Verify contrast** for every text/background combination using a contrast checker.
1. **Document edge cases** (e.g., the darker brand variant for text contexts).
1. **Reference everywhere.** No hardcoded values in components.
1. **Update once.** When tokens change, the whole product reflects the change.

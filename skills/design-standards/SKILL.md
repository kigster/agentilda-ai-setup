...
1. **Sketch hierarchy.** Before writing markup, identify the primary action, secondary actions, and supporting content. Confirm the scan order makes sense.
1. **Build mobile-first.** Lay out for the smallest target viewport before scaling up.
1. **Apply tokens.** All values come from the token set. No hardcoded colors, sizes, or spacing.
1. **Run contrast checks.** Every text-on-background combination passes AA. Every UI element passes 3:1.
1. **Test at viewport breakpoints.** 375, 768, 1024, 1440 minimum. Confirm nothing breaks.
1. **Run the pre-ship checklist** in [`references/preship-checklist.md`](references/preship-checklist.md).

...
1. **Contextual-when-needed, not tooltip-spam or one-and-done.** Help surfaces at moments of friction.
1. **Trigger logic sound.** Event-based primary, with time and state modulation.
1. **Tour architecture matches usage.** Library of micro-tours preferred for most products.
1. **Contextual placement non-intrusive.** Help findable when wanted, ignorable when not.
1. **Dismissal mechanics honest.** Standard close, click-outside dismiss, no auto-dismiss before reading.
1. **Completion tracking instrumented.** Per-user state, per-tour status.
1. **Re-trigger logic respectful.** Completed tours do not re-show; dismissed tours respected.
1. **Power-user vs new-user differentiation.** Help calibrated to user state.
1. **Mobile parity.** Help works on the devices the audience uses.
1. **Maintenance discipline.** Tours updated alongside product changes; quarterly audit.
1. **Adoption as success metric.** Not just tour completion; feature adoption is the metric that matters.

...
1. **Pull baseline data.** Referring domains, anchor text, link types, lost links, competitor overlap.
1. **Map the profile across 5 dimensions.** One section per dimension.
1. **Identify red flags.** Velocity spikes, anchor concentration, low-DR concentration, suspect TLDs, repeated footprints.
1. **Build the toxic list (if needed).** Use the criteria in [`references/toxic-link-criteria.md`](references/toxic-link-criteria.md).
1. **Build the reclamation list.** Lost links worth recovering via outreach.
1. **Build the gap list.** Domains linking to peers, prioritized by relevance.
1. **Synthesize.** Profile health verdict, top 3 risks, top 3 opportunities.
1. **Recommend.** Disavow (rare), reclamation outreach, link building targets, monitoring cadence.
1. **Document and hand off.** Outputs feed `seo-offpage` and `seo-audit-orchestration`.
...
1. **Profile snapshot.** Key metrics: referring domains, DR distribution, anchor mix, velocity.
1. **5-dimension analysis.** One section per dimension with findings.
1. **Toxic link list.** If needed. Disavow file ready format.
1. **Reclamation list.** Lost links worth recovering, with contact paths.
1. **Gap list.** Top 50 domains linking to competitors but not target.
1. **Recommendations.** Disavow (Y/N), reclamation outreach plan, link building targets.
1. **Methodology.** Data sources, pull dates, criteria used.

...
1. **Conversions with definition and window visible.** Not just a count; the definition of what counts as a conversion and the attribution window applied. Without this, the count is unreadable.
1. **Attribution breakdown.** Last-click vs view-through vs modeled. The mix of how the conversions were credited.
1. **Frequency.** Impressions per unique user. The fatigue early-warning system.
1. **Audience saturation.** Where the platform exposes it. A flat audience-saturation curve means there is room to scale; a steep curve means efficiency is dropping.
1. **Time series.** Daily breakdown to spot novelty effects, fatigue, day-of-week patterns, and exogenous variance.
1. **Cost metrics in clear currency.** CPC, CPM, CPA, ROAS with the math defined and the currency labeled. Do not assume USD.
1. **Conversion path data.** Touchpoints before conversion, where available. Tells you whether a campaign is a closer or an opener.
1. **Filters, segments, and exports.** Without these, the panel is a brochure, not a tool.
...
1. **Platform-reported vs reality.** View-through, modeled conversions, conversion windows.
1. **Attribution model.** Pick one and read the others as sanity checks.
1. **Multi-platform reconciliation.** Sum-of-platforms is always inflated.
1. **ROAS vs LTV horizon.** Short-term metric, long-term impact.
1. **Cohort vs daily.** Cohort tells the quality story; daily tells the volume story.
1. **Statistical noise.** Weekly variance, day-of-week, seasonal, exogenous.
1. **Incrementality.** What would have happened without the spend.
1. **Geo and holdout testing.** The honest causal test.
1. **Self-attribution bias.** Platforms claim credit they do not deserve.
1. **Decision rule.** Pre-committed scale up, hold, or pull back.
1. **Single source of truth.** Warehouse over platform reporting for board metrics.

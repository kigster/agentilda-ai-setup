...
1. **Set the criteria first.** Ordered, before any product is examined, so the criteria cannot quietly bend toward a favored pick.
1. **Gather per tier.** Verify specs at the source. Pull owner corpora at scale and note sample sizes. Collect named expert sources, including the disagreements.
1. **Synthesize.** Findings per criterion, drawbacks per pick, splits surfaced.
1. **Write the methodology block** from what was actually done, not from what sounds good.
1. **Match the markup to the claims.** ItemList and Article by default; Review only where tier 4 is true.
1. **Maintain.** Date updates, state what changed, upgrade pieces when hands-on arrives, and mark the upgrade.

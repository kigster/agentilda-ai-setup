...
1. **Earned-progressive-disclosure, not tutorial-overload or skip-friendly-empty.** Each step earns one step closer to value.
1. **Step architecture sound.** Each step moves the user closer to value; non-contributing steps cut.
1. **The ah-ha moment engineered.** Single visible value moment, action-tied, in the first session.
1. **Progressive disclosure applied.** Show only what is needed now; defer the rest.
1. **Skip mechanics honest.** Skip exists but does not produce an empty product.
1. **Resume mechanics work.** Skipped users have a path back; instrumented for follow-through.
1. **Drop-off measurement instrumented.** Per-step tracking from launch.
1. **User-type variations balanced.** 2-3 variants max; over-differentiation avoided.
1. **Mobile parity.** Wizard works on the devices the audience uses.
1. **Activation as success metric.** Not just completion rate; the metric is post-wizard product engagement.
1. **Maintenance discipline.** Wizard updated alongside product changes; quarterly audit.

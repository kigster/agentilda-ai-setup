...
**Step 1: Primary upgrade ask** (matched to trigger). **Step 2 (if declined): Downsell** (smaller commitment alternative). **Step 3 (if declined): Soft acknowledgment** ("No problem; here is the link to plans whenever you are ready."). **Step 4 (later): Cross-sell** (when the user has continued use signals).

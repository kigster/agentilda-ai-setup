...
1. **Value-triggered-upgrade, not paywall-everywhere or free-forever-trap.** Paywalls at moments of demonstrated value.
1. **Trigger-moment design sound.** Paywalls connect to specific user-demonstrated value.
1. **Paywall presentation matches trigger intensity.** Strong triggers warrant modal; soft triggers inline.
1. **Copy connects to the trigger.** Specific value-prop tied to what the user just did.
1. **Upsell, cross-sell, downsell logic.** When primary upgrade is declined, secondary paths exist.
1. **Win-back flows designed.** Lapsed users have a clear re-engagement path.
1. **Churn prevention upstream.** Upstream signals trigger prevention; downstream upgrade flow is one of multiple defenses.
1. **Plan structure clear.** Tier count appropriate; each tier has clear audience-fit.
1. **Pricing-page coordinates with upgrade flow.** Plan structure decisions reflected on the pricing page.
1. **Conversion as success metric.** Not just paywall display; downstream conversion to paid.
1. **Maintenance discipline.** Plans, paywalls, win-back flows updated alongside product and pricing changes.

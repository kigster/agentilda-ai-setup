...
1. **Run automated scans.** Document findings per page.
1. **Manual keyboard pass.** Test all priority flows.
1. **Screen reader pass.** Test with at least one combination.
1. **Visual checks.** Contrast, zoom, color blindness, motion.
1. **Cognitive checks.** Reading level, error handling, time limits.
1. **Score against WCAG.** Per success criterion (level A, AA, AAA).
1. **Prioritize findings.** Critical (blocks users), Important (degrades experience), Minor (polish).
1. **Write the report.** Use the template in [`references/audit-report-template.md`](references/audit-report-template.md).
1. **Build a remediation plan.** Sequenced fixes with effort and impact estimates.
...
1. Executive summary
1. Methodology (tools used, pages tested, screen readers used)
1. Findings by WCAG principle
1. Critical findings (P0) with specific URLs and fixes
1. Important findings (P1)
1. Minor findings (P2)
1. Polish (P3)
1. Remediation roadmap (sequenced and prioritized)
1. Appendices (full automated scan results, keyboard navigation notes, screen reader notes)

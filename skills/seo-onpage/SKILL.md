...
1. **Render the page.** View it as a user would. Read the content top to bottom.
1. **View the rendered HTML.** Inspect the actual served markup, not just the visual page. Check `<title>`, `<meta>`, headers, and schema in the source.
1. **Run the 8-dimension framework.** Score each, note specific fixes.
1. **Prioritize.** Group fixes into Critical (broken or missing), Important (suboptimal), and Nice-to-have (polish).
1. **Write the report.** Use the template in [`references/audit-template.md`](references/audit-template.md).
1. **Offer to draft fixes.** If the user wants, draft the new title, meta, headers, or copy directly.
...
1. Score across 8 dimensions
1. Critical fixes
1. Important fixes
1. Nice-to-have polish
1. Drafted replacements (if requested)

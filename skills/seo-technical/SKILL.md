...
1. **Confirm access.** What can you actually see (HTML, robots, sitemap, search console, server logs, staging)?
1. **Crawl.** Use a crawler to enumerate URLs and statuses. Sample if the site is huge.
1. **Run the 6-layer framework.** Score each, note specific issues with example URLs.
1. **Cross-reference.** Search console for what's actually indexed. Compare to sitemap and crawl output.
1. **Prioritize.** Critical (blocks indexing or causes traffic loss), Important (suboptimal), Nice-to-have (polish).
1. **Write the report.** Use the template in [`references/audit-template.md`](references/audit-template.md).
...
1. 6-layer score
1. Critical issues (with example URLs)
1. Important issues
1. Nice-to-have polish
1. Implementation roadmap (sequenced)

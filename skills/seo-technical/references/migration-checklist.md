...
1. Verify search console is processing the new sitemap
1. Spot-check top 50 lost-traffic URLs for content parity (does the new page actually answer the same query?)
1. Check robots.txt and meta robots for accidental noindex
1. Check for hreflang errors if international
1. Submit affected URLs for re-indexing manually
1. Audit internal linking - are key pages still well-linked?

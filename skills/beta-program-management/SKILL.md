...
1. **Beta type matches signal need.** Closed/open, alpha/beta/RC, internal/external, time-bounded/open-ended.
1. **Participants match the post-launch profile.** Match the user the GA serves.
1. **Cohort size calibrated to signal need.** Smaller for bugs; larger for behavioral signal.
1. **Onboarding contract clear.** Expectations on both sides documented.
1. **Feedback channels structured.** 3-5 channels; signal not noise.
1. **Mid-beta triage active.** Feedback acted on during the beta.
1. **Communication keeps participants engaged.** Updates on what was heard and what is changing.
1. **Graduation criteria explicit.** Critical bugs cleared, friction addressed, behavioral validation, performance under load, support readiness, positive signal.
1. **Wind-down treats participants well.** Transition clarity, recognition, thank-you.
1. **Postmortem feeds practice.** Each beta improves the next.
1. **Honest decisions on graduation.** Either criteria met (graduate) or not (extend or reconsider). No tired-of-the-beta graduation.

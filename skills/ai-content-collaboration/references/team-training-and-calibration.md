...
1. Each editor writes a one-paragraph assessment per piece: voice, brief adherence, AI tells, fact accuracy, overall quality.
1. The team reads all assessments together.
1. Differences in interpretation surface; the team reconciles to a shared standard.
1. The session output is an addendum to the policy doc capturing the calibration consensus.

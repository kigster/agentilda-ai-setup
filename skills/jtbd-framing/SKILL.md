...
1. **Job statement structure: situation, motivation, outcome.** Specific at all three levels.
1. **Struggling moments ground the jobs.** Specific recent moments, not abstractions.
1. **Hire criteria identified.** What makes users adopt the product over alternatives.
1. **Fire criteria identified.** What makes users switch away (or would).
1. **Functional, emotional, social dimensions surfaced.** All three; not functional only.
1. **Jobs derived from data, not workshop output.** Job statements emerge from interview data, not from whiteboard sessions.
1. **JTBD applied where it earns its keep.** Discovery, prioritization, positioning. Not as ritual.
1. **Segment differences in jobs surfaced.** When the same product is hired for different jobs by different segments.
1. **Decisions traceable to JTBD analysis.** Each major decision can be explained through the job framing.
1. **JTBD vocabulary not substituted for analytical work.** Vocabulary without analysis is ritual.
1. **Honest assessment of where JTBD does not help.** Some questions do not benefit from the framing; force-fitting produces ritual.

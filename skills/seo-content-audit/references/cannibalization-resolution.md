...
1. Look for queries where the same site appears more than once in the data
1. Check rank stability: if a query bounces between URLs week to week, that is the signal
1. Internal: search your own site (`site:yourdomain.com "exact query"`) and see how many results appear
...
1. Pull the unique content from the loser pages, integrate into the winner.
1. 301 redirect the loser URLs to the winner.
1. Update internal links pointing at loser URLs to point at the winner.
1. Submit the winner URL for re-crawling.
1. Monitor rankings for 30 to 60 days.
...
1. Rewrite titles, meta, H1, and intros to reinforce the distinct intent
1. Update internal links to use distinct anchor text
1. Cross-link the two pages so each acknowledges the other
1. Update any sitemap or hub page that lists them
...
1. 301 redirect the losers to the winner.
1. Pull any unique content from the losers and integrate into the winner if useful.
...
1. Confirm the canonical of the secondary page points at itself (not the primary), since canonical and noindex serve different purposes
1. Verify in search console after a week that the noindex took effect
1. Maintain the user-facing functionality of the page
...
1. **Backlinks.** Highest count of referring domains usually wins.
1. **URL quality.** Cleaner slug, fewer parameters, shorter, descriptive.
1. **Internal links in.** More inbound internal links signals existing site structure.
1. **Page age.** Older URLs often have accumulated trust signals.
1. **Content quality.** If all else is tied, pick the one with better content (or the one easier to expand).

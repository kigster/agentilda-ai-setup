...
1. **Affinity mapping.** Cluster observations into themes. Physical sticky notes or digital equivalents.
1. **Find patterns.** Themes that appear across multiple participants are signal. One-off observations are interesting but weaker.
1. **Identify insights.** An insight is more than a theme. It's a non-obvious finding that explains a why or implies a so what.
1. **Test the insight against the data.** If the insight only fits some interviews, it's a hypothesis, not an insight.
1. **Distinguish signal from noise.** A belief that 1 of 8 participants holds may be noise. A belief 6 of 8 hold is signal.
...
1. **Pick the method.** Match to the question.
1. **Plan logistics.** Timeline, budget, recruit, tools, team.
1. **Recruit.** Start early. Slow recruits delay everything.
1. **Pilot.** Run 1 to 2 sessions before the main batch. Refine the guide.
1. **Conduct.** Stay disciplined to the guide while staying open to surprises.
1. **Synthesize.** Don't wait until all sessions are done; start mid-way.
1. **Communicate.** Multiple formats. Multiple audiences.
1. **Track impact.** Did decisions change because of the research? If not, the research failed regardless of quality.
...
1. **Interview guide** - `interview-guide-[topic].md`
1. **Findings doc** (after synthesis) - `research-findings-[topic].md`
1. **Highlight reel** (video, separately produced)

...
1. **Pre-test questions** (3 to 5 min). Background context, current behavior with similar products.
1. **Task 1** (5 to 10 min). Describe task. Have participant attempt while thinking aloud.
1. **Post-task questions** (1 to 2 min). What was easy/hard? Anything confusing?
1. **Repeat for tasks 2, 3, 4** (typically 3 to 5 tasks per 60-minute session).
1. **Overall debrief** (5 to 10 min). General reactions, comparisons to alternatives, anything else.
1. **Close** (2 min).
...
1. **Cluster.** Issues that are the same root problem.
1. **Severity.**
...
1. **Recommendations.** For each issue, propose specific fixes.
...
1. **Design tasks.** 3 to 5 specific, realistic, goal-framed tasks.
1. **Choose moderated vs unmoderated.** Match to stage and depth needed.
1. **Recruit.** Specific to audience.
1. **Pilot.** 1 to 2 sessions before main batch. Refine tasks if needed.
1. **Run.** Follow the protocol. Stay disciplined.
1. **Synthesize during, not just after.** Patterns emerge by session 4 or 5.
1. **Report.** Multiple formats - written report + highlight clips.
1. **Track fixes.** Every critical issue should have an owner and date.
1. **Re-test after fixes.** Verify the fix worked, didn't introduce new issues.
...
1. **Task script** (per session) - `usability-tasks-[topic].md`
1. **Findings report** (after synthesis) - `usability-findings-[topic].md`
1. **Highlight clips** (separately produced)

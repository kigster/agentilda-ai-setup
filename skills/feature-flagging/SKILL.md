...
1. For each flag: identify owner, last evaluation, type, recommended action.
1. Triage in a quarterly meeting: keep, remove, or investigate.
1. Open one PR per flag for code-side removal. One per PR is reviewable; bundling ten is not.
1. Delete from the platform after the PR merges.
...
1. **Naming.** Typed prefix, owner prefix, semantic name. Avoid vague.
1. **Lifecycle planning.** Birth and death committed at creation. Removal date in metadata.
1. **Targeting taxonomy.** User, account, request, or time-based. Avoid volatile attributes.
1. **Rule simplicity.** AND, OR, NOT only. Three nested clauses means the taxonomy is wrong.
1. **Rollout match.** Strategy matches the risk profile. Percentage for low risk, cohort for medium, geo for compliance, time-based for coordinated launches.
1. **Ramp discipline.** One peak hour at each percentage step. No four-hour rollouts.
1. **Stale flag cadence.** Quarterly cleanup. One PR per removal. Make removal part of the launch checklist.
1. **Permission tiering.** Viewer / editor / approver / admin. Fewer admins than approvers, fewer approvers than editors.
1. **Environment promotion.** Dev to staging to production with review gates. Production rules are not authored in production.
1. **Conflict audit.** Two flags on the same surface = conflict. Detect via shared-key audit; coordinate cross-team via the experiment registry.
1. **Performance budget.** Bulk evaluation, caching, server-side SDKs for sensitive logic. 5 ms total budget for fifty flag checks.
1. **Branch testing.** Both code paths covered by tests. Transition behavior documented.
1. **Observability.** Every flag evaluation logged. Every rollout dashboarded. Errors correlated with flag value during incidents.

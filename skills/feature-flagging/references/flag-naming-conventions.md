...
1. New flags follow the convention. No exceptions.
1. Existing flags get renamed when touched: when the rule changes, when the rollout advances, when ownership transfers, when removal is scheduled.
1. Quarterly cleanup includes a "rename batch" of the 5-10 worst-named flags, especially ones that are confusing during the cleanup itself.
1. After 6-12 months, the platform stabilizes on the convention.
...
Bad: `enterprise_users_audit_log` (the "enterprise_users" is targeting) Better: `perm_audit_log` (the flag is a permission flag for the audit log; the rule targets enterprise users)

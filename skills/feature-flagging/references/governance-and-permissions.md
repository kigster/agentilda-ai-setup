...
1. Editor tests the change in staging (the rule applies, the metric moves as expected).
1. Editor opens a change request in the platform (most platforms have a built-in change-request workflow; if not, a Slack channel with a bot suffices).
1. The request includes: rule diff, rollout plan, abort criteria, link to relevant ticket/PR, link to staging test results.
1. An approver reviews. If clean, applies. If not, comments and the editor revises.
1. The change applies in production with the audit trail attached.
...
1. The group includes the on-call rotation; on-call inherits Approver for the duration of their shift.
1. Emergency overrides apply with the standard audit trail; the reason field includes the incident reference.
1. After the incident, the override is reviewed in the post-mortem.

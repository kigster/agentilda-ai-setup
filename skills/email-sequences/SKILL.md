...
1. **Define the goal.** What does success look like at the end of the sequence?
1. **Map the journey.** What does the recipient need to know, do, or feel between trigger and goal?
1. **Outline emails.** One purpose per email. Sequence them across appropriate timing.
1. **Draft email 1.** Subject, preview, opening, body, CTA.
1. **Draft remaining emails.** Each builds on the previous.
1. **Test the sequence.** Send to yourself. Read on mobile. Time the gaps.
1. **Set up triggers.** Configure in the email service provider.
1. **Measure.** Open rate, click rate, completion rate per email. Sequence completion rate.

### For a single broadcast

1. **Define the audience.** Specific segment, not the whole list.
1. **Define the goal.** One thing this email accomplishes.
1. **Draft subject + preview.** Generate 5 to 10 variations.
1. **Draft body.** Single message, clear CTA.
1. **Test on mobile.** Most opens are mobile.
1. **Schedule.** Day and time matched to audience behavior.
1. **Measure.** Open rate, click rate, downstream conversions.

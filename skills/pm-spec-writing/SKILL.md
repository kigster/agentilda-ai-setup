...
1. **Who specifically benefits?** Be precise. "Users" is not specific. "First-time visitors who don't convert" is.
1. **What is the success metric?** How will you know it worked? Pick one primary metric.
1. **Why now?** What changed that makes this the right time to build it? If "nothing changed," it might not be the right time.
...
1. **Scope.** Plot the work on the impact/effort grid.
1. **Pick the right format.** Feature spec for new features, dev brief for tactical work, bug report for defects.
1. **Write the spec.** Use the template format. Fill in every section. Empty sections are flags.
1. **Define done.** Verify steps must be unambiguous. "Test it" is not a verify step.
1. **Get buy-in.** Walk through the spec with whoever will build it before they start.
1. **Sequence.** Identify the smallest shippable increment.

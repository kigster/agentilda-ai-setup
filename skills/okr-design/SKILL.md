...
1. **Objectives are outcomes, not outputs.** What we are trying to achieve.
1. **Few objectives, well-focused.** 2-4 per team per quarter.
1. **Key results are measurable.** Specific numbers or testable thresholds.
1. **Key results are outcome-aligned.** Achieving them moves the objective forward.
1. **Key results are within team influence.** Not entirely dependent on external factors.
1. **3-5 key results per objective.** Single key results are fragile; many dilute focus.
1. **Cascading explicit but not over-constrained.** Team OKRs ladder up but with autonomy on how.
1. **Scoring honest, not rounded.** 0.0-1.0 reflects actual outcomes.
1. **Recalibration rare.** OKRs hold absent strategic shift or major disruption.
1. **Review cadence weekly + mid-quarter + end-of-quarter.** Tactical, then strategic, then scoring.
1. **OKRs separate from roadmap items and metrics.** Each serves a different purpose.

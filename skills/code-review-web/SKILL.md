...
1. **Pick the depth.** Quick scan for a small PR. Full review for a major change. Deep dive for a production incident.
1. **Run through the 5 dimensions.** Note issues by severity (blocker, important, minor).
1. **Check stack-specific patterns.** Reference the appropriate stack guide.
1. **For incidents:** identify the smallest hypothesis-driven fix. Reproduce locally if possible.
1. **Write the review.** Use the template in [`references/review-template.md`](references/review-template.md) for formal reviews.
...
1. **Check hosting build and function logs.** The exact failing line is usually here.
1. **Identify the last working version.** `git log --oneline` and check recent commits.
1. **Reproduce locally.** Confirms it's a code issue and not an environment issue.
1. **Check environment variables.** Especially after deploys or DNS changes.
1. **Check cache state.** Force a cache invalidation before concluding it's a code bug.
1. **Make the minimal fix.** Big refactors during incidents create more incidents.
1. **Verify in production.** Check the actual fix worked, not just that the deploy succeeded.
1. **Document.** What was the root cause? What would have prevented it? File the learnings.

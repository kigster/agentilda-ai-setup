...
1. **Actionable-segmentation, not clickbait or vanity.** The result places the taker into a category with a specific recommendation.
1. **Question architecture sound.** 5-12 questions; each contributes to segmentation; phrased plainly without leading.
1. **Scoring algorithm fits the segmentation.** Direct mapping, weighted scoring, multi-dimensional, or branching as appropriate.
1. **Result categories distinguishable and balanced.** 4-8 segments; each meaningfully different; each receives a meaningful share of takers.
1. **Segment naming memorable and brand-consistent.** Names the taker can describe to a friend.
1. **Recommendations specific to each segment.** No two segments share the same recommendation; each recommendation is valuable to that segment.
1. **Lead capture honest.** Email exchange for personalized delivery, not for seeing the result.
1. **Result delivery in context.** Recommendation appears with the result, not buried 3 emails later.
1. **Drop-off measured.** Question-by-question drop-off tracked; questions that lose audience flagged for review.
1. **Lead quality measured by segment.** Downstream conversion tracked per segment; some segments may be more valuable than others.
1. **Audit cadence.** Periodic review of segment balance, recommendation alignment, and conversion-by-segment.

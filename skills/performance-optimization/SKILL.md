...
1. **Identify the worst offender.** LCP, INP, or CLS - which is failing? Focus there first.
1. **Diagnose specifically.** Browser dev tools (Performance tab, Network tab, Coverage tab). Identify the actual cause of the metric failure.
1. **Plan fixes.** Per identified issue, plan a specific change. Estimate impact and effort.
1. **Implement.** One fix at a time where possible. Easier to measure impact.
1. **Re-measure.** After each major fix, re-run Lighthouse and check Real User Monitoring.
1. **Iterate.** Performance is rarely solved in one pass. Plan for ongoing monitoring and fixes.
...
1. Executive summary
1. Methodology (tools, conditions, sample pages)
1. Current state (Core Web Vitals, Lighthouse scores, RUM data)
1. Critical issues (Core Web Vitals failures)
1. Important issues (sub-optimal but not failing)
1. Polish (further-than-required wins)
1. Remediation roadmap (sequenced)
1. Performance budget recommendations
1. Monitoring plan

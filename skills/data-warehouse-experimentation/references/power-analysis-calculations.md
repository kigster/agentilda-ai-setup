...
1. **Sample size per arm.** Number of users in each variant.
1. **Alpha (significance level).** Default 0.05. The false-positive rate.
1. **Power.** Default 0.8. One minus the false-negative rate. The probability of detecting a real effect.
...
1. **Expand the audience.** If the experiment is targeting only paid users (10 percent of the user base), consider running on all users. The result is generalizable to a broader population, but the intervention may need to work for the broader population too.
1. **Accept inconclusive.** Some experiments cannot be powered with the available sample. Document the MDE; design the experiment to detect a larger effect or accept that it will be inconclusive. The discipline of saying "we cannot reliably answer this with our sample" is hard but honest.

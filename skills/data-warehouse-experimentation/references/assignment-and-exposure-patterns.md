...
1. **Stable across the experiment lifecycle.** Do not change the salt mid-experiment. Changing the salt re-randomizes existing users; the analysis becomes uninterpretable.
1. **Versioned.** When semantics change, append `_v2`. The old experiment data remains queryable; the new salt produces independent assignments.

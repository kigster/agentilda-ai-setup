...
1. **mSPRT.** Always-valid p-values that survive peeking.
1. **Confidence sequences.** Always-valid confidence intervals via the Howard et al construction.
1. **Group sequential designs.** Pre-specified interim analyses with calibrated alpha-spending.
...
1. At each interim point, compute the test statistic.
1. If the statistic exceeds the boundary, stop the experiment and reject the null.
1. If the statistic does not exceed any boundary by the final analysis, fail to reject (treat as inconclusive or as "no effect detected").

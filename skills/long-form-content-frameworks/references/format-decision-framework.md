...
1. **Can the team pay the format-specific tax?** If the format demands specifics and the case is too confidential to share specifics, the format does not fit. If the format demands conviction and the writer wants to hedge, the format does not fit.
1. **Is there an original contribution?** Whitepapers, manifestos, and research reports particularly require original contribution. Definitive guides and tutorials can be valuable as comprehensive synthesis without original argument.
1. **Does the audience expect this format?** Some audiences expect whitepapers (B2B technical), some manifestos (founder-led brands), some research reports (analyst-influenced markets). Format choice signals genre-fit to the audience.
1. **Is the production capacity available?** Long-form tutorials with verified sequential steps cost meaningful verification time. Research reports cost primary-research budget. Format choice must match capacity, or the format-tax goes unpaid and the piece reads as the format-it-pretended-to-be.

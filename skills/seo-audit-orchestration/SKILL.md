...
1. **Confirm Ahrefs MCP access.** Verify the workspace has the target property and recent data.
1. **Pull all data.** Run the gather list in phase 2. Document freshness.
1. **Run each sub-audit.** Use the child skills. Each produces its own findings doc.
1. **Synthesize themes.** 5-7 themes max. Each ties to an impact and a fix.
1. **Prioritize.** Impact/effort matrix. Quick wins surface to the top.
1. **Draft the rollup.** Use the template. Executive summary first.
1. **Review with the team.** Pressure-test conclusions before stakeholder readout.
1. **Deliver.** Walk stakeholders through. Get commitment to next 90 days.
1. **Schedule the next one.** Audits decay. Rerun on a cadence.
...
1. **Audit charter** (scope, goal, methodology).
1. **Themes and recommendations** (5-7 themes, each with what, why, size of prize, fix).
1. **Sub-audit appendix** (linked outputs from each child skill).
1. **90-day roadmap** (sequenced work with owners and targets).
1. **Methodology notes** (data sources, pull dates, caveats).

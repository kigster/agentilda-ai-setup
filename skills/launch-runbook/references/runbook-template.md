...
1. Re-enable maintenance mode (if applicable)
1. Revert DNS records to previous values (if changed)
1. Re-deploy previous code version
1. Restore database from backup (if migrations were destructive)
1. Verify previous version is functioning
1. Disable maintenance mode
1. Run smoke tests on rolled-back state
1. Announce rollback complete

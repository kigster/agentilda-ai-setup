...
1. **Search intent classification.** Informational, navigational, commercial, or transactional. Plus the dominant SERP format check (article, listicle, comparison, video, tool). The intent decides what kind of piece this is; the format decides the shape.
1. **Target audience.** Specific role and sophistication level. Not "anyone interested in X." A senior data engineer at a 500-person SaaS company is a target; "data people" is not.
1. **Job to be done.** The problem the reader is trying to solve when they land on the piece. One sentence. Specific.
1. **Word count / depth target.** Calibrated to the SERP, not to a content quota. If the top 10 are all 1,200 words, briefing 3,000 wastes effort; if the top 10 are all 4,000 words, briefing 1,500 produces a piece that cannot rank.
1. **Heading structure outline.** H2s and H3s explicit, ordered logically. The outline is where the writer reads the brief most carefully; spend time here.
1. **Required entities, statistics, citations.** For AEO and GEO, the named tools, methods, experts, and statistics the piece must cover. The entity gap is the agent's prioritization signal.
1. **Internal linking strategy.** 3 to 5 pages this piece should link out to with anchor text; 1 to 3 existing pages that should link back to this piece after publish.
1. **External proof points.** Required citations, sources, expert quotes. Not optional.
1. **Anti-patterns.** What not to do. Off-topic tangents the writer might wander into, off-brand voice, AI clichés the brief explicitly forbids by listing them in a do-not-use list (the standard set of overused corporate-speak adjectives, throat-clearing openers, and verbs that signal AI generation).
1. **Success criteria.** What good looks like. Rank for the target keyword in 90 days. Get cited by ChatGPT for the cluster within 60 days. Drive 50 trial signups in the first 30 days. Measurable, time-bound, tied to the program goal.
1. **Author voice and tone reference.** Link to the brand-voice guide. If the piece needs a tone shift from the default voice, note it here in one sentence.
...
1. Identify entities mentioned across multiple pages: named tools, named methods, named experts, named data sources, named statistics.
1. Identify entities mentioned by 1 to 2 pages but not in your existing content. This is the entity gap.
1. Add the gap entities to the brief as required coverage.
...
1. **Search intent + dominant SERP format.** Match the SERP, not your priors.
1. **Target audience + JTBD.** Specific role and problem, not "anyone interested in X."
1. **Word count calibrated to SERP.** Not to a content quota.
1. **Heading structure.** Explicit H2s and H3s, ordered to match reader mental model.
1. **Entity coverage.** Entities the SERP top 10 mention; gap entities added explicitly.
1. **Internal linking.** 3 to 5 outbound links specified; 1 to 3 inbound updates queued.
1. **External proof points.** Citations and sources required.
1. **Anti-patterns.** What not to do, including AI clichés and off-brand voice.
1. **Success criteria.** Measurable, time-bound, tied to original program goals.
1. **Brief versioning + approval.** Single approver; version bumps for material changes.
1. **Brief stays under 2 pages.** Every field earns its keep; remove fields that do not.

...
1. Writer asks clarifying questions before drafting (or the agent surfaces ambiguities as the first response).
1. Writer or agent acknowledges the success criteria explicitly.
1. First draft references the brief: which fields the draft addresses, which fields it could not address, why.
1. Editor reviews against the brief, not against personal taste.

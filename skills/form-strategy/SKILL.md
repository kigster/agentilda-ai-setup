...
1. **Time-based detection.** Reject submissions completed in under 2-3 seconds (bots) or after very long delays (suspicious sessions).
1. **Rate limiting.** Reject if the same IP submits too many times.
1. **CAPTCHA as a last resort.** Modern invisible CAPTCHAs (hCaptcha, reCAPTCHA v3, Turnstile) are low-friction. Old image CAPTCHAs are conversion-killers.
1. **Behavioral signals.** Did the cursor move? Was there scroll? Modern services track this.
1. **Content filtering.** Reject obvious spam content (links, foreign-language content if your audience is local, common spam words).
1. **Server-side review.** A queue rather than direct delivery to inboxes for high-spam-target forms.

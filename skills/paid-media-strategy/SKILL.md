...
1. **Channel fit.** Intent matches offer. Search for capture, Meta and TikTok for creation, LinkedIn for B2B precision.
1. **Budget shape.** Brand vs performance, baseline vs test, primary vs secondary channel, daily vs lifetime.
1. **Audience strategy.** Prospecting, retargeting, exclusion. Treated as separate strategies.
1. **Bid strategy.** Manual, max conversions, tCPA, tROAS, max conversion value. Matched to data state.
1. **Campaign type.** Right type for the platform and the goal.
1. **What not to spend on.** Branded beyond defensive, untargeted display, wrong geos, off-hours, low-converting devices, never-convert audiences, tired creative.
1. **Creative testing.** Within campaign and across campaign, plus refresh cadence.
1. **Frequency capping.** Explicit caps and creative rotation.
1. **Attribution reality.** Single source of truth, platform metrics for in-flight only.
1. **Decision rule.** Pre-committed scale up, hold, or pull back at known thresholds.

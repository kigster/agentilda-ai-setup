...
1. **Run discovery.** Pull seeds, competitor exports, search console data, SERP inspections. Aim for 200 to 500 candidates.
1. **Deduplicate and clean.** Remove obvious junk, brand misspellings, irrelevant terms.
1. **Classify by intent.** Mark each keyword.
1. **Cluster.** Group into topical clusters. Aim for 20 to 50 clusters.
1. **Score each cluster** on opportunity, difficulty, and strategic fit.
1. **Prioritize.** Rank by composite score. Identify the top 10 to 20 clusters to produce first.
1. **Output.** Use the template in [`references/keyword-research-template.md`](references/keyword-research-template.md).

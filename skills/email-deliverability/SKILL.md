...
1. Get the `include:` value or IP for each
1. Combine into a single SPF record
1. Verify lookup count is under 10
1. Use `-all` for hard fail (or `~all` if rolling out gradually)

**DKIM fix order:**

1. Generate a new selector per sending service
1. Publish the public key in DNS
1. Configure the ESP to sign with the private key
1. Verify with a test send (check headers for `dkim=pass`)

**DMARC fix order:**

1. Publish DMARC with `p=none` initially (monitoring mode)
1. Set up an aggregate report endpoint (use a DMARC analytics service or your own)
1. Watch reports for at least 2-4 weeks
1. Identify any legitimate senders failing alignment; fix them
1. Move to `p=quarantine` with `pct=10`, gradually increase
1. Move to `p=reject` once confidence is high

...
1. If the CI is all-positive (lower bound greater than zero, e.g., [+1%, +4%]), there is a real effect. Now evaluate magnitude: is the lower bound large enough to be worth the implementation cost?
1. If the CI is all-negative (upper bound less than zero, e.g., [-5%, -1%]), there is real harm. Kill the test.
1. If the CI straddles zero but is narrow (e.g., [-0.5%, +0.5%]), this is a real null result. The effect is small enough to call essentially zero. Useful information; do not ship the change for "lift" reasons (you found none) but do not panic about harm either.
1. If the CI straddles zero and is wide (e.g., [-5%, +8%]), the test is inconclusive. The data is consistent with a moderate win, no effect, or a moderate loss. Run longer, run bigger, or accept that the question cannot be answered at the available traffic.
...
1. **Confidence interval reading.** Width matters more than center. Five practical decision rules cover the cases where CI excludes zero, includes zero narrowly, and includes zero widely.
1. **P-value semantics.** The probability of seeing the lift if the null is true. Not the probability that the treatment works. Always read alongside the CI.
1. **Multiple testing corrections.** Bonferroni for family-wise control, BH for false discovery rate. Pre-register primary metrics and segments; treat the rest as exploratory.
1. **Sequential testing math.** Always-valid p-values and anytime-valid CIs survive peeking. Fixed-horizon p-values inflate false positives under daily monitoring.
1. **CUPED variance reduction.** Same point estimate, narrower CI. Use whenever pre-experiment data is available and informative.
1. **Heterogeneous treatment effects.** Pre-register segments. Treat post-hoc segment effects as hypotheses for follow-up tests, not as ship signals.
1. **Ratio metrics.** Use delta method or bootstrap. Naive proportion estimators understate variance and inflate false positives.
1. **Bayesian vs frequentist.** Pick one per experiment and stick with it. Both produce similar ship decisions when the experiment was designed correctly.
1. **Network effects and SUTVA.** Marketplace and social products often violate SUTVA. Cluster randomization, switchback, or geographic experiments when interference is suspected.
1. **Dashboard reconciliation.** Different denominators, time windows, external effects, and metric definitions explain most disagreements between BI dashboards and experiment panels.
1. **Long-term effect estimation.** Holdouts, geo experiments, and diff-in-diff for effects that emerge over thirty to ninety days.
1. **Common interpretation failures.** A pattern catalog of the most frequent ways smart teams ship wrong things.
1. **The discipline of inconclusive.** The hardest call is "we do not have enough signal to ship." It is often the right call.

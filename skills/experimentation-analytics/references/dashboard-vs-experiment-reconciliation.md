...
1. **Same denominator?** Confirm the experiment population and the dashboard population. Often they differ.
1. **Same time window?** Confirm the experiment window and the dashboard window are aligned (or explain why they are not).
1. **Any external events?** List campaigns, launches, outages, holidays during the test window. The dashboard reflects them; the experiment cancels them.
1. **Any pipeline lag?** Confirm both numbers are using the same attribution and the same data freshness.
1. **Any selection effects?** Who got enrolled and how? If enrollment was non-random (by feature flag check, by cohort), the experiment population is not representative of the dashboard population.

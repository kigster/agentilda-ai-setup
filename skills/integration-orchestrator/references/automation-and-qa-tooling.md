...
1. **The blocker reason is documented** in the ticket comment with specifics: which test failed, which file was the locus, which assertion didn't pass, what the agent already tried.
1. **If the failure surfaced something unrelated to the current task,** an adjacent observation gets filed in the project triage queue.
1. **The human is paged via the configured channel.**
...
1. **The agent does NOT continue retrying.** It documents the state and waits. Retrying is the path that turns a single failure into a context-burning loop.

...
1. **Per-phase gate specs.** What enters, what exits, who approves, the measurable pass criteria for each gate including the QA verification gate.
1. **Lock-point register.** What becomes immutable when, where the locked artifact lives, who can request a change, and what re-review work a change triggers.
1. **Tool-stack implementation guide.** Concrete setup for Jira, Linear, Notion, Figma, and GitHub as applicable, with MCP commands or CLI invocations for the parts that automate.
1. **QA verification gate spec.** Which automated tests must pass, which MCPs run them, where evidence attaches, what happens on failure.
1. **Handoff specs.** Artifacts transferred per handoff, locked items, change-request protocols, adjacent-observation routing.
1. **Cross-skill dependency graph.** Which skills run in which phase, what each skill consumes, what each produces.
1. **Risk register.** Cadence-specific risks with mitigations.

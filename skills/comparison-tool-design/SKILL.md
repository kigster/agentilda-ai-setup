...
1. **Honest-comparison-with-guidance, not feature-list-dump or hidden-recommendation.** Genuine compare plus opinionated rec.
1. **Axis selection.** 8-12 decision-relevant axes; cut decoration features.
1. **Default-comparison logic.** Honest defaults; not bias-flattering.
1. **Recommendation engine designed.** Visible; defended; not the only path.
1. **Filter and toggle UX.** Filters that help; not filters for customization theater.
1. **Methodology disclosed.** Source data, axis weighting, audience definition.
1. **Mobile parity.** Tool works on the devices the audience uses.
1. **Maintenance discipline.** Comparison stays current as options change.
1. **Honest about competitor strengths.** When competitors win on an axis, say so.
1. **Audience-fit measured.** Per-segment conversion through the tool.
1. **Conversion as success metric.** Not just engagement; downstream choice and retention.

...
If clicks and sessions diverge significantly: the issue may be tracking, not traffic. If year-over-year shows the same pattern: the change is seasonal, not anomalous.
...
1. Recommend the lowest-risk action that helps in either case.
1. Identify the data that would distinguish the hypotheses.
1. Propose a monitoring plan to gather that data.

...
1. **Validate the data.** Layer 1 checks. Rule out tracking and seasonality.
1. **Localize.** Layer 2. Segment until the pattern is clear.
1. **Page-level dive.** Layer 3 on the most affected pages.
1. **Technical check.** Layer 4. Recent deploys, robots, canonicals, redirects.
1. **External check.** Layer 5. Algorithm updates, competitors, industry.
1. **Build the hypothesis.** State the cause as a single sentence.
1. **Validate the hypothesis.** Find the evidence that confirms or refutes it. See [`references/diagnosis-checklist.md`](references/diagnosis-checklist.md).
1. **Action plan.** Specific fixes mapped to specific evidence.
1. **Communicate.** Write up the diagnosis. Stakeholders want clarity, not exhaustive analysis.
...
1. **The symptom.** Charts and segment breakdowns.
1. **Layer-by-layer findings.** What each layer ruled in or out.
1. **Root cause hypothesis.** Single statement with evidence.
1. **Action plan.** Ordered fixes with owners and timelines.
1. **Recovery forecast.** Realistic expectations on timeline and ceiling.
1. **Monitoring plan.** What to watch for confirmation of recovery.

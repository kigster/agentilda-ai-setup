...
1. **Best research type for that decision.** Discovery (interviews lead), validation (survey or quantitative leads), friction-mapping (support and in-app feedback lead), positioning (sales call analysis leads).
1. **Composition.** Which secondary types triangulate the primary?
1. **Capacity.** What can be collected within the time and budget the decision allows?
1. **Existing data check.** Has the team already collected research that would answer the question? Mining existing artifacts often produces value before commissioning new research.

...
1. Each function lead confirms their readiness (10 minutes).
1. Open Q&A captured in the decision log (10 minutes).
...
1. Confirm comms are gated on the health check (the social post will not auto-fire if the rollout is paused).
1. Confirm support is staffed for the launch window.
1. Confirm escalation contacts are reachable.
1. Open the post-launch monitoring channel.

...
1. Was the rollback the right call (almost always yes; even false positives that catch nothing teach the team about the system).
1. What is the path to re-launch (fix the bug, test it, validate the test, schedule a new rollout).
1. What did we learn that updates the next launch's monitoring.

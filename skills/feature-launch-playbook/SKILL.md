...
1. **Engagement.** Did users who tried it come back? The stickiness question. First use is necessary but not sufficient.
1. **Outcome.** Did the metric the spec said this would move actually move? The effect question. The hypothesis the spec made.
1. **Side effects.** Did any other metric move that was not supposed to? The safety question. Sometimes the feature works but breaks something else.
...
1. **Position it.** Target user, problem, promise, proof points, anti-positioning.
1. **Align internally.** Single launch brief, distributed two to four weeks before launch.
1. **Plan customer comms.** Channel mix, calendar, owners, gates.
1. **Enable sales.** Battlecard, demo, training, deal coaching (B2B only).
1. **Ready support.** Training, FAQ, escalation paths, monitoring access.
1. **Pick a rollout strategy.** All-at-once, gradual, flag-gated, or phased.
1. **Define monitoring.** Metrics, alerts, rollback triggers. Pre-defined, not in-the-moment.
1. **Sequence comms execution.** Gated on health checks. Never auto-fire if the rollout is paused.
1. **Tie measurement to spec hypotheses.** Adoption, engagement, outcome, side effects.
1. **Iterate post-launch.** Weekly triage. Distinguish marketing, usability, value, and segment problems.
1. **Declare success or failure on a stable trend.** Not the launch-week spike.

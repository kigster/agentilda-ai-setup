...
1. **Audience and stage segmentation defined.** 3-5 audiences x 3 stages; the matrix the architecture serves.
1. **Entry-point architecture mapped.** Each entry point routed to expected segments and stages.
1. **Tool-to-funnel mapping documented.** Each tool serves specific segments at specific stages.
1. **Nurture sequence architecture per segment.** Sequences vary by audience and stage; not one sequence for everyone.
1. **Cross-tool data flow integrated.** Context travels across tools; the audience experiences continuity.
1. **Architecture-level metrics tracked.** Cross-tool conversion, sequence-to-tool conversion, segment-level downstream conversion.
1. **Funnel iteration discipline.** Refine continuously; redesign deliberately; avoid both extremes.
1. **Tool portfolio balanced.** Multiple tools serving different segments; not over-reliant on any single tool.
1. **Audience-fit honest.** The architecture serves the segments the brand can actually serve; out-of-fit audiences are filtered or routed elsewhere.
1. **Maintenance ownership clear.** Someone owns the architecture; quarterly review is calendared.
1. **Expansion plan defined.** When adding new tools, the architecture says where they fit; new tools earn their place rather than being added decoratively.

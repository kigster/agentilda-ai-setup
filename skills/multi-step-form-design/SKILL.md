...
1. **Genuinely-staged, not kitchen-sink or progress-theater.** Each step represents a coherent unit of work from the user's perspective.
1. **Step architecture sound.** Identity, context, need, detail, confirmation. Low-friction first; commitment compounds.
1. **Progress indicator honest.** Reflects actual remaining work; does not shift mid-form.
1. **Conditional logic adds value.** Removes irrelevant fields without adding maintenance overhead.
1. **Save-and-resume mechanics fit the form.** Offered when the form length warrants; trust clearly communicated.
1. **Validation strategy matched to form.** Per-step for required fields; end-only for nuanced validation.
1. **Drop-off measurement instrumented.** Per-step tracking from launch.
1. **Mobile experience tested.** Form works on the devices the audience uses.
1. **Field count audited.** Each field necessary; non-essential fields removed.
1. **Final-step clear.** Submission action obvious; what happens after submission communicated.
1. **Maintenance discipline.** Quarterly review of conversion, drop-off, validation, and conditional logic.

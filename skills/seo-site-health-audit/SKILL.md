...
1. **Tier the URLs.** Use organic traffic data. Tag every affected URL as Tier 1, 2, or 3.
1. **Categorize each issue by mechanism.** High or low impact mechanism.
1. **Estimate effort per fix type.** Group similar fixes into one effort estimate.
1. **Apply the triage matrix.** Assign P0-P3 or Park.
1. **Cluster the fixes.** Group fixes that share an effort: one template change can resolve hundreds of issues.
1. **Build the backlog.** P0 first, P1 next, etc. Add fix descriptions, owners, expected impact.
1. **Add measurement.** What metric will confirm the fix worked? Define before shipping.
1. **Hand off.** Output feeds the development backlog and `seo-audit-orchestration`.
1. **Re-crawl after fixes.** Confirm resolution. Update the backlog.
...
1. **URL tiering.** How URLs were classified and counts per tier.
1. **Issue categorization.** Counts by mechanism category, by tier, by priority band.
1. **Prioritized backlog.** P0-P3 ordered. Each item has: issue, affected URLs, fix description, effort, expected impact, owner.
1. **Fix clusters.** Groups of issues that share a fix. Highest leverage at the top.
1. **Measurement plan.** Per fix or cluster, what proves it worked.
1. **Methodology.** Crawler used, date, scope, caveats.

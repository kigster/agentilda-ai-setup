...
1. **(Density-bearing)** A menu page, not a PDF, with name, description, and price visible per item, reachable in one click from the home.
1. Item descriptions in ingredient-and-method voice, not adjective stacks; signature dishes marked.
1. A reservation action that is on-site, not a third-party widget iframe, available from the chrome on every page.
1. **(Density-bearing)** A short chef-perspective or chef-partner passage, first-person, two short paragraphs at most. The story of the kitchen, not a brand history.
1. Address and hours visible above the fold; full hours line-by-line on the home or visit page.
1. JSON-LD Restaurant with hasMenu, servesCuisine, acceptsReservations, openingHoursSpecification, and aggregateRating.
1. Cancellation, late, and allergy policies stated openly on the reservation page.
1. **(Density-bearing)** One named-publication press pull-quote OR a visible aggregate rating (count plus number), placed near the reservation CTA. Not a logo wall.
1. A phone-as-fallback path: a visible phone number for visitors who would rather call.

...
1. **Card sort or interview** to surface mental models.
1. **Draft the sitemap.** Hierarchy, page types, cross-references.
1. **Define URL patterns.** One pattern per content type.
1. **Design navigation.** Primary, secondary, utility, footer, breadcrumbs.
1. **Build taxonomy.** Categories (controlled, small) and tags (open, large).
1. **Validate labels.** Tree test or closed card sort with target users.
1. **Document.** Use the template in [`references/ia-document-template.md`](references/ia-document-template.md).
1. **Hand off to design and development.** IA decisions inform navigation components, URL routing, and taxonomy implementation.
...
1. Executive summary
1. Audience and mental models (synthesized)
1. Sitemap (hierarchical diagram)
1. URL structure (per content type)
1. Navigation specification (primary, secondary, utility, footer, breadcrumbs)
1. Taxonomy (categories and tag governance)
1. Labels (validated wording for navigation, categories, content types)
1. Implementation notes for design and development

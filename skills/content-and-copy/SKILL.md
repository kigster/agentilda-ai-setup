...
1. **Outline.** Structure first. Headlines and section purposes only. Skip prose until the structure is sound.
1. **Open with the hook.** Write the opening before anything else. If the hook isn't working, the rest won't either.
1. **Draft for substance.** First draft is about getting the substance down. Voice comes in editing.
1. **Edit for voice.** Read aloud. Tighten sentences. Cut padding. Apply brand voice.
1. **Edit for clarity.** Each paragraph: does it earn its place? Each sentence: is the meaning unambiguous?
1. **Pre-publish checks.** Headers in order, no skipped levels, internal links present, image alt text written, SEO basics applied (title, meta, slug).

### For editing existing content

1. **Read the whole piece first.** Do not edit on first read. Understand it before changing it.
1. **Identify what's working.** Note the strong sections so you don't damage them.
1. **Identify what's broken.** Voice mismatches, weak structure, vague claims, padding.
1. **Edit by priority.** Structure first (move sections, cut sections). Then substance (strengthen claims). Then voice. Then sentence-level polish.
1. **Read aloud.** Catch awkward phrasing the eye misses.

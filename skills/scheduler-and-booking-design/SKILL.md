...
1. **Qualified-fast-path, not any-time-friction or interrogation-gate.** Just enough qualification to set up the call well.
1. **Qualification fields earn their place.** 3-5 fields; each routes or preps.
1. **Availability logic matches team structure.** Round-robin, weighted, fit-based, or hybrid.
1. **Routing connects qualification to rep tier.** High-fit leads to senior reps.
1. **Prep automation delivers data to reps.** CRM enrichment, calendar invite, Slack brief.
1. **Confirmation and reminders reduce no-shows.** 24-hour and 1-hour typical.
1. **Reschedule is friction-low.** Self-serve; preserves data.
1. **No-show flow honest.** Re-engagement attempts limited; respect non-response.
1. **Mobile parity.** Booking works on the devices the audience uses.
1. **Conversion to sale as success metric.** Not just booking rate; downstream call-to-sale conversion.
1. **Maintenance discipline.** Routing rules updated as team changes; quarterly audit.

...
1. **Statistical rigor.** CUPED, sequential testing, delta method for ratio metrics, multiple testing corrections. Verify at the sales-call level; do not assume.
1. **MCP availability.** Current capability and trajectory. Weight higher if your team is AI-forward.
1. **Feature flag integration.** Same platform or separate. Decide once and live with the implications.
1. **Analytics depth.** Funnels, retention, cohorts in the same surface. Decides whether you need a second tool for the analytics layer.
1. **Governance.** Permissions, audit, approval workflows, environment promotion. Non-negotiable for regulated industries.
1. **Cost shape.** Events vs seats vs warehouse compute. Match the shape to your usage trajectory.
1. **Team fit.** PM-led, engineer-led, data-led, marketing-led. The platform's defaults will pull your culture in the direction of its primary user.
1. **Onboarding cost.** Time to first experiment for a new team member. Faster is better; varies wildly across platforms.
1. **Migration cost.** What is the exit plan if you choose wrong. Higher migration cost means higher confidence required at decision time.
1. **Multi-platform fit.** Does this layer cleanly with platforms you already have, or does it create overlap.

...
1. **Metrics differ between platforms.** "Activation rate" computes one way in Statsig and another way in PostHog. Stakeholders see two numbers and trust neither.
1. **Operational overhead exceeds the value of platform diversity.** Engineers spend more time keeping platforms in sync than they save by using each platform's strengths.
...
1. **A leadership disagreement surfaces a "which one is the source of truth" debate.** Multi-platform is meant to support different surfaces, not different versions of the same answer. If the same answer has two values, consolidation is the fix.
1. **The team is spending more than 10% of its experimentation operational time on platform coordination.** That time should be on experiments, not platform plumbing.

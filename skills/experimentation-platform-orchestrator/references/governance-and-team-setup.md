...
1. **Author.** Create and modify experiments and flags in non-production environments. Default for PMs and engineers shipping experiments.
1. **Production.** Modify production targeting. Reserved for senior engineers, leads, or designated experiment owners.
1. **Admin.** Manage users, billing, integrations. Reserved for engineering leadership and platform owners.
...
1. A second person reviews. The reviewer is not the author.
1. The change is logged with actor, timestamp, before-and-after, and reason.
...
1. **Approval theater.** Changes go through an approval workflow but reviewers do not actually review. The fix is to require a substantive comment on every approval, not just a click.
1. **Audit log drift.** The audit log exists but no one reads it. The fix is to surface audit log highlights (high-risk changes, unusual targeting modifications) in a weekly digest.

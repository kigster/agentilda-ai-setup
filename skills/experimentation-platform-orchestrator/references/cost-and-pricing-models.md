...
1. **Event volume tiers are negotiable.** The published pricing is the starting position. Custom contracts at scale always run below it.
1. **Bundle features for unit-cost reduction.** Combining experiments and feature flags in one contract often costs less than the same features separately.

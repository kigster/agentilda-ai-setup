...
1. **Triage.** System owner(s) decide:
...
1. **Design.** Component is designed in the system's style. Includes all states, variants, and accessibility requirements.

1. **Review.** System owner reviews against system standards. Iterate.

1. **Build.** Component is built in code (and Figma if not built there first).

1. **Document.** Usage doc, API doc, examples, anti-patterns.

1. **Release.** Versioned release. Changelog entry. Announcement to consumers.
...
1. **Migration period.** Defined window (typically 1 to 2 quarters) for consumers to migrate. Migration guide published.

1. **Removal.** Component removed. Code deleted. Documentation archived (not deleted, for historical reference).

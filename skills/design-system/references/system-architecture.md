...
1. Migration guide
1. Codemod or scripted migration path where possible
1. A timeline for sunset (typically 1 to 2 quarters)
...
1. **Tokens first.** Establish the token foundation. Migrate hard-coded values gradually.
1. **Primitives second.** Replace ad-hoc components with system primitives. Highest-impact ones first (button, input, modal).
1. **Patterns third.** Once primitives are established, codify patterns.
1. **Migrate over multiple quarters.** Rip-and-replace fails. Coexistence with gradual migration succeeds.

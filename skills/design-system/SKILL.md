...
1. **Identify the duplicates.** Same component built 5 different ways across the product. These are your high-value consolidation targets.
1. **Define foundations.** Token set, with both base and semantic layers. Document each.
1. **Audit elements.** From the inventory, identify the actual elements (buttons, inputs, etc.) and reduce variants to a managed set.
1. **Build the element library.** Figma + code. Document each element.
1. **Identify priority components.** The 10 to 15 components that appear most often. Build those first.
1. **Document patterns.** Page-level templates that show the system in use.
1. **Establish governance.** Owner, contribution model, review cadence.
1. **Roll out.** Migrate existing surfaces to the system progressively.
...
1. **Map gaps.** Where the system is incomplete. Where teams build outside the system because the system can't serve their need.
1. **Map fragmentation.** Where the system has divergent implementations (Figma vs. code, web vs. mobile, multiple teams).
1. **Identify decay.** Components that have drifted from the documented standard.
1. **Prioritize fixes.** Foundation gaps first. High-use component drift second. Rarely-used component cleanup last.
1. **Plan rollout.** Major changes need migration paths.

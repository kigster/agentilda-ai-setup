...
1. **Object-action format.** Noun then verb. `video_played`, `form_submitted`, `email_opened`. Reading the event name aloud should describe what happened.
1. **Granular but not redundant.** Track distinct user actions, not button clicks. Fire `checkout_completed` once at the moment of completion, not `submit_button_clicked` plus `checkout_completed`. UI events are noise; semantic events are signal.
...
1. **Time windows matter.** Most funnels use a 1 to 30 day window from the first event. Document the window explicitly; "users who completed signup AND activated" is meaningless without "within 14 days."
1. **Drop-off interpretation is hard.** Eighty percent of users dropping at step 2 might be the funnel design (audience is wrong), might be the audience (timing is wrong), or might be the product (genuine drop-off). Investigate before declaring.
1. **The anchor event pattern.** Every funnel starts with a high-intent action: signup, trial start, key feature use. Not a vanity event like `page_view` or `session_start`. Vanity-event-anchored funnels show 99% drop-off and tell you nothing.
...
1. **Each metric has a known data source.** Not "from the warehouse" but "from `fct_orders`, last 30 days, joined to `dim_customers`."
1. **Each metric has known caveats.** Modeled iOS conversions excluded. Internal employees excluded. Test users in QA accounts excluded.
1. **Each metric is reproducible.** The same query, run tomorrow, produces the same number for the same time period.
...
1. **Property design.** Event-level vs user-level. Type discipline. ISO 8601 timestamps everywhere.
1. **Naming conventions.** Pick one and enforce it. snake_case wins by default.
1. **Schema versioning.** Additive vs breaking. `_v2` suffix during transitions. Data contract in code.
1. **Identity stitching.** Anonymous to authenticated, cross-device, cross-tool. One canonical identity layer.
1. **Funnel design.** Anchor on high-intent events. Document time windows. No auto-firing events as steps.
1. **Cohort definitions.** Reusable, versioned, apples-to-apples. Defined in code or saved in the tool.
1. **Retention measurement.** Bracket over N-day for stability. Compare cohorts at matched ages.
1. **North Star.** Captures user value. Hard to game. One NSM, three to five inputs, five to ten health metrics.
1. **Dashboard hygiene.** Definitions, sources, owners, refresh cadence. Stale dashboards get deprecated.
1. **Instrumentation debt.** Every PR includes instrumentation. Quarterly audits.
1. **Single source of truth.** Warehouse for board metrics. Tool for in-flight optimization. Reconcile when they disagree.

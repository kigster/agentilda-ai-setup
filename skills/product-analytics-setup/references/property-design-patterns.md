...
1. **Schema drift when the value changes.** A user upgrades mid-session. Some events have `subscription_tier: free`; later events have `subscription_tier: pro`. Reporting becomes ambiguous about which value to use.
1. **Reporting confusion.** Analyst writes "WHERE subscription_tier = 'pro'" thinking they are filtering users. The query actually filters events, returning a different cohort.

...
1. **Guided-configuration, not infinite-options or canned-bundles-only.** Smart defaults plus meaningful constraints plus escape hatches.
1. **Default configuration sensible.** User adjusts from default; does not build from zero.
1. **Constraint logic prevents invalid combinations.** Hard, soft, implication, capacity constraints.
1. **Constraint communication clear.** Why constraint engaged; what user can do.
1. **Real-time pricing visible.** Price updates with choices; breakdown available.
1. **Validation helpful.** Inline; specific; offers paths.
1. **Save-and-share works.** Multi-session; shareable; B2B-friendly.
1. **Configurator-to-cart handoff preserves state.** Cart shows configuration.
1. **Mobile parity.** Configurator works on the devices the audience uses.
1. **Conversion as success metric.** Not just completion rate; downstream purchase.
1. **Maintenance discipline.** Configurations updated as product changes; quarterly audit.

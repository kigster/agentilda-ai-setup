...
1. **Brief-adherence first.** Cheapest, fastest, highest-value gate.
1. **Fact-accuracy as halt-condition.** AI hallucinations do not ship to readers.
1. **Voice consistency including mid-piece sampling.** Long pieces drift; sample throughout.
1. **AI-content audit.** Pattern recognition for AI tells, hallucinations, voice drift.
1. **Structure and clarity.** Lede, sectioning, even-ish length, specific endings.
1. **SEO and AEO compliance.** Answer paragraphs, schema, entity coverage.
1. **Internal linking and schema validation.** Outbound links live, anchor text varies, schema validates.
1. **Sampling at scale.** Programmatic and high-volume sets need sampling, not full-audit.
1. **Threshold gating.** Failure rate above threshold halts generation; document breaches.
1. **Single QA owner per piece.** Accountability not diffused across committee.
1. **Sequencing.** Brief-adherence to fact-accuracy to structure to AI-audit to voice to SEO/AEO to linking to schema.

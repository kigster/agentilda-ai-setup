...
1. Compare each sample to the brand voice doc's vocabulary, rhythm, stance, register.
1. If start and end match brand voice while middle samples regress to generic, the piece has voice drift.
...
1. Sample 2 paragraphs from the middle. Does either match AI tells? Does the voice match the brand voice doc? Flag.
1. Read the last 200 words. Does the closing match any of the 11 AI tells? Flag.
1. Search the piece for em-dashes; count. More than 5 in a 1,500-word piece is suspicious.
1. Search for forced bilateral framing patterns. Count.
1. Verify every statistic, quote, case study, citation, and product claim per the fact-accuracy methodology.
1. Sample 3 to 5 internal links; verify they go where the brief specified and the targets exist.

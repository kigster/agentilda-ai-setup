...
1. **Anchor text variation.** Same-target links use varied anchor text.
1. **Targets live.** No 404s, no deprecated pages.
1. **Link count in band.** 3 to 7 for typical, 8 to 15 for pillar.
1. **Self-cannibalization.** No competition with existing pieces.
1. **Inbound links queued for post-publish update.** The brief queued 1 to 3 existing pages that should add links to this piece after publish; the QA log queues those updates.
...
1. **Schema validates.** Run through a validator; resolve errors before publish.
1. **Schema matches content.** No false claims (false ratings, false step counts).
1. **Optional fields populated.** Where data exists, fields populate; the page signals depth.
1. **Multiple schema types when appropriate.** Article + FAQPage + BreadcrumbList is common for pillar pieces with FAQ sections.

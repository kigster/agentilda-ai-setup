...
1. **Accessibility.** Passes axe or equivalent automated checks.
1. **Keyboard navigation.** Tab, Enter, Escape, arrow keys all work as expected.
1. **Component logic.** Props produce expected output. Events fire correctly. (Unit tests.)
1. **Integration.** Component works inside its expected parent contexts.
...
1. **Sketch the anatomy.** Name the parts.
1. **List variants and states.** Match the design system or define new ones if needed.
1. **Design the API.** Required props, optional props, children, events. Type it.
1. **Build the markup with semantic HTML.** Choose the right elements. Avoid generic `<div>` for interactive things.
1. **Style with tokens.** No hardcoded colors, spacing, or sizes.
1. **Add interaction.** Focus management, keyboard handlers, ARIA.
1. **Add states.** Hover, focus, active, disabled, loading, error.
1. **Test.** Automated accessibility, keyboard navigation, visual regression.
1. **Document.** Usage, API, examples, anti-patterns.

...
1. **Pull the inventory.** Every URL, with publish date, primary topic, organic clicks, ranked keywords.
1. **Pull competitor coverage.** Top pages and content clusters from each competitor.
1. **Classify each existing page.** Healthy / thin / outdated / decaying / off-topic.
1. **Identify missing topics.** Cross-reference competitor top pages and keyword gap output.
1. **Score each opportunity.** See [`references/content-decision-matrix.md`](references/content-decision-matrix.md).
1. **Cluster.** Group related missing topics into pillar concepts.
1. **Decide actions.** Create / update / merge / redirect / prune for each opportunity.
1. **Sequence the roadmap.** Quarter by quarter, with effort estimates and traffic forecasts.
1. **Hand off.** Output feeds `content-strategy`, `content-and-copy`, and `seo-onpage`.
...
1. **Content inventory snapshot.** Counts by health classification.
1. **Missing topics.** Prioritized list with cluster mapping and traffic forecast.
1. **Thin coverage.** Pages to deepen, with recommendation for each.
1. **Outdated content.** Pages to refresh, with refresh scope.
1. **Decaying content.** Pages to diagnose and act on, with hypothesis for each.
1. **Cannibalization findings.** Page pairs to merge.
1. **Action roadmap.** Quarter by quarter, with effort and forecast.
1. **Methodology.** Data sources, classification criteria, caveats.

...
1. **Refresh.** Update facts, examples, screenshots, year references.
1. **Update.** Deeper rework. New sections, restructured argument, expanded scope.
1. **Merge.** Combine with another page. Redirect the loser.
1. **Redirect.** Page has no path forward. Redirect to closest relative.
1. **Prune.** Remove from sitemap, return 410 or noindex.
...
1. More ranked keywords with meaningful volume
1. Better URL (cleaner, more descriptive, shorter)
1. Higher current position for the head term

...
1. **Channel taxonomy known.** Support, NPS, in-app, sales, social, councils. What each surfaces.
1. **Channel-source weighting per decision.** Different decisions weight channels differently.
1. **Categorization and tagging at scale.** Taxonomy that emerges from data; tooling that supports volume.
1. **Frequency-intensity matrix.** Each feedback item assessed on both dimensions.
1. **Synthesis cadence.** Daily critical, weekly patterns, monthly review, quarterly strategic.
1. **Cross-channel triangulation.** Patterns that appear across channels are stronger.
1. **Closing the loop with users.** When feedback drives change, communicate.
1. **Drift detection.** Patterns over time reveal changes.
1. **Tooling that scales.** Multi-channel aggregation, pattern surfacing, integration.
1. **Specific feedback informs specific decisions.** Decisions traceable to specific signals.
1. **Honest assessment of source bias.** Each channel's bias acknowledged in synthesis.

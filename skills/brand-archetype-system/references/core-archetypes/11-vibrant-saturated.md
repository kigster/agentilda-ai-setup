...
1. **Saturation calibration**: keep all colors at similar saturation. Mixed high-saturation and low-saturation produces incoherence.
1. **Type restraint**: the type should not also be expressive. Clean type plus expressive color is the formula; expressive type plus expressive color is chaos.
1. **Color carries meaning**: in the strongest applications of this archetype, color is functional (different colors signal different content types). Use this rather than decorative color.
1. **Accessibility**: contrast ratios must work despite saturated palette. Test every color combination against WCAG AA at minimum.

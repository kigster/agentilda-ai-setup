...
1. **Display size**: do not pull this back to "reasonable." If the hero is 56px, you have made an Editorial Restrained brand instead. Commit to 100px+ or pick a different archetype.
1. **Voice**: easy to slip into bro-culture. Pull back the swagger by writing honest, not performed. The visual is bold; the voice is direct, not bombastic.
1. **Image strategy**: pick type-as-visual or one strong photo. Mixed imagery dilutes the impact.
1. **Use the contrast**: panel-by-panel section transitions are core. A single-color page is not Bold Confident.

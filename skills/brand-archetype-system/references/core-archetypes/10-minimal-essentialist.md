...
1. **Type family pick**: Helvetica reads neutral and Swiss; Söhne reads contemporary and sharp; Suisse reads precise and minimal; Inter reads accessible and modern. Pick based on temperature, not novelty.
1. **Color discipline**: black plus white plus one accent maximum. Brands that "just add one more color" frequently undermine the archetype.
1. **Photography**: product-on-white only. Lifestyle photography pulls toward Warm Conversational or Luxe Considered.
1. **Voice**: write honest, not poetic. Minimal Essentialist voice that tries to be evocative reads as Luxe Considered; minimal voice that is too plain reads as Editorial Restrained.

...
1. **Mono density**: dial up for developer brands (mono in hero, mono in nav, mono in labels) and dial down for prosumer brands wanting only the precision signal.
1. **Dark mode**: required. Skip this archetype if the brand cannot ship a dark mode.
1. **Photography**: if the brand needs human imagery, blend with Editorial Restrained or Warm Conversational instead of forcing it into Technical Precise.
1. **Voice density**: stay specific. Vague claims in this archetype read as worse than vague claims elsewhere, because the visual is promising specificity.

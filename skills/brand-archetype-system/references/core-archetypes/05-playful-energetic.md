...
1. **Palette discipline**: defining 4 to 6 saturated hues that compose together is the hardest part. Spend real time here; the rest of the archetype is downstream.
1. **Motion**: the archetype needs motion to feel right in product. Static-only execution reads as toy-without-charm.
1. **Voice**: write to one specific reader, not "users." The archetype falls apart when the voice tries to be funny without commitment.
1. **Restraint**: even Playful Energetic needs editorial moments. Pages that are 100% playful all the way through exhaust the reader.

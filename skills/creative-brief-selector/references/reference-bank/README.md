...
1. If the build discovered any new live references in addition (because the bank's references were stale or the build needed more breadth), append them to the positive references list in the same file with a one-line why.
1. Commit the file edit as part of the build PR.
...
1. Fills the file with three to six positive references discovered during the build's step 3.
1. Fills negative references with one to two categories or named sites that the build should not pull from.
1. Commits the file as part of the build PR.

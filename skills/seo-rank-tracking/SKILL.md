...
1. **Pull starting data.** Existing ranked keywords, Search Console queries, competitor overlap.
1. **Build the tracked set.** 4 buckets, 200-500 total keywords for most properties.
1. **Segment with tags.** Topic, funnel stage, page mapped, etc.
1. **Configure tracking.** Set up Ahrefs Rank Tracker projects with tags and locations.
1. **Capture baseline.** Day-one positions and SERP composition.
1. **Define alert thresholds.** By bucket. Wire to notification system.
1. **Build the dashboard.** See [`references/dashboard-template.md`](references/dashboard-template.md).
1. **Set the cadence.** Weekly review of money + brand. Monthly review of all. Quarterly rebuild of opportunity bucket.
1. **Iterate.** Drop dead keywords. Add new ones. Recalibrate thresholds based on noise.
...
1. **Tracked keyword set.** All keywords with bucket and tag assignments.
1. **Baseline snapshot.** Position, SERP composition, CTR per keyword.
1. **Alert configuration.** Thresholds by bucket, routing.
1. **Dashboard layout.** What charts, what filters, what is visible.
1. **Review cadence.** Weekly, monthly, quarterly responsibilities.
1. **Methodology notes.** Country and device settings, refresh frequency, data caveats.

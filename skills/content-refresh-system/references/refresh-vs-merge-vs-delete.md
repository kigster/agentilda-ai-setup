...
1. **Is there a sibling piece competing for the same query?** If yes, evaluate merge. The merged piece has a strong chance of outperforming both original pieces.
1. **Is the topic load-bearing for the brand?** If no, refresh disposition is unlikely to be the right answer; consider merge into a stronger sibling or delete.
1. **Has refresh been tried recently without recovery?** If yes, the next disposition should be merge or full-rewrite-with-new-positioning, not another refresh attempt.
1. **What is the cheapest disposition that addresses the actual problem?** Refresh is often defaulted to because it feels less destructive; merge and delete are sometimes the cheaper paths to the actual outcome.

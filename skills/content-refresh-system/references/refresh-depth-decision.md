...
1. **Re-scope explicitly.** When the editor discovers the planned depth is wrong, halt the refresh and re-scope formally. The capacity allocation gets adjusted; the timing gets adjusted; the team is aware the refresh became a bigger project.

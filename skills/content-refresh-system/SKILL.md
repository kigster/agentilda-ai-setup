...
1. **Signal categories named.** Traffic decay, ranking drops, factual staleness, SERP intent shift, content drift.
1. **Audit cadence committed.** Quarterly audit plus continuous monitoring; both running.
1. **Prioritization matrix applied.** High-value-decaying first; low-value-decaying audited for merge or delete.
1. **Depth matched to signal.** Light edit, substantial revision, full rewrite, structural redesign.
1. **Disposition decided per piece.** Refresh, merge, or delete; not all decay warrants refresh.
1. **Single ownership per refresh.** Accountable owner runs the work to ship.
1. **Capacity allocation explicit.** Refresh ratio of editorial capacity is a real number, planned.
1. **Re-promotion built into the workflow.** Schema, visible date, search-engine resubmit, audience re-share.
1. **Per-refresh tracking.** Signals, depth, outcomes logged for pattern detection.
1. **Effectiveness measurement.** Recovery tracked at 30 and 90 days; patterns inform program decisions.
1. **The-refresh-that-did-not-work review.** Failed refreshes get structured review; lessons compound.

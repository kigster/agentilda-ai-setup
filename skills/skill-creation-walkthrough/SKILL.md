...
1. **When to use it** (the trigger phrases or situations).
1. **Edge cases** with "Also triggers when..." (catches less obvious situations).
1. **Useful for...** (audience or use cases, helps disambiguation).
...
1. **Description draft**: the 2-4 sentence description for the YAML frontmatter.
1. **SKILL.md body**: the full body of the skill, following the standard structure.
1. **Reference plan**: a list of reference files to create with one-line descriptions.
1. **First reference file**: at least one reference written end to end as a starting point.
1. **Test plan**: 3-5 test prompts that should trigger the skill, plus 2-3 that should not.

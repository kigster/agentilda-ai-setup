...
1. **Primary triggers**: when to use it (the obvious cases).
1. **Edge cases**: secondary triggers introduced with "Also triggers when...".
1. **Audience or scope**: who this serves, introduced with "Useful for...".
...
1. Write down 3 prompts that should not.
1. In a fresh conversation with the skill installed, try each prompt.
1. Check whether the skill loaded.
1. Adjust the description until trigger and non-trigger prompts behave correctly.

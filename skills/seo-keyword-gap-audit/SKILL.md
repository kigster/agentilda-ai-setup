...
1. **Pull keyword data.** For each property: organic keywords, top pages, traffic estimates.
1. **Build the gap matrix.** Label each keyword with which property ranks where.
1. **Filter to the pure gap cell.** Strip out the other cells for now.
1. **Validate intent.** For each keyword, confirm the SERP intent matches what the target can produce.
1. **Score by opportunity.** Use the formula. See [`references/opportunity-scoring-rubric.md`](references/opportunity-scoring-rubric.md).
1. **Cluster.** Group related keywords into topics. One topic, one piece of content.
1. **Map to existing content.** Some gap keywords are addressable by updating an existing page rather than creating new.
1. **Sequence.** Build the prioritized list with action (create / update / merge / new pillar).
1. **Hand off.** Output feeds `content-strategy` and `seo-onpage`.
...
1. **Competitor set.** Who, why selected, SERP overlap evidence.
1. **The gap matrix.** Counts and patterns across the four cells.
1. **Pure gap analysis.** The opportunity list, scored and banded.
1. **Topic clusters.** Groups of related keywords mapped to content concepts.
1. **Action map.** Create / update / merge decisions per cluster.
1. **Quick wins.** Top 5-10 highest-confidence opportunities.
1. **Strategic plays.** High-impact, longer-horizon investments.
1. **Methodology.** Data sources, scoring weights, caveats.

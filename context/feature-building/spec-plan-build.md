...
> [!IMPORTANT]
> **This file is generated.** Run `spec-plan-build docs -o context/SPEC-PLAN-BUILD.md`
...
Every project keeps its plans in a `.plans/` directory at its root.
...
| ⭐️ | **Ready** | `ready` | `spec.md`, `plan.md` | specified and planned; ready to be implemented |
| 🟡 | **In Progress** | `wip` | `pull-requests.md` | at least one pull request raised, not all merged |
| ✅ | **Done** | `done` | `pull-requests.md` | every pull request merged |
| ⭕️ | **Blocked** | `blocked` | `blocked.md` | cannot proceed; an engineer or the CTO must decide something first |
| 🅱️ | **Product Blocked** | `product_blocked` | `blocked.md` | cannot proceed; a product manager must decide something first |
...
| ⛔️ | **Rejected** | `rejected` | `rejected.md` | never going to be implemented; a terminal state |

"Files required" is a **minimum**, not an exact match: a ⚪️ folder that
has grown a `plan.md` still satisfies ⚪️, and is ⭐️ anyway. That is
why `resync dirs` moves a folder to the furthest state its contents justify
rather than only fixing outright lies.

Two states share their requirements on purpose. ⭕️ Blocked and
🅱️ Product Blocked both mean "a human must decide before this can
move"; *which* human is recorded nowhere but the emoji. So nothing re-derives
one from a folder's contents — otherwise every ⭕️ would silently become
🅱️ the first time anything resynced.
...
| :--- | :--------- | :---------------- |
| ⚪️ New | ⭐️ ⭕️ 🅱️ ☢️ ⛔️ | ⭐️ Ready |
| ⭐️ Ready | 🟡 ⭕️ 🅱️ ☢️ ⛔️ | 🟡 In Progress |
| 🟡 In Progress | ✅ ⭕️ 🅱️ ☢️ ⛔️ | ✅ Done |
| ✅ Done | 🟡 | — |
| ⭕️ Blocked | ⚪️ ⭐️ 🟡 ☢️ ⛔️ | — |
| 🅱️ Product Blocked | ⚪️ ⭐️ 🟡 ☢️ ⛔️ | — |
| ☢️ Deferred | ⚪️ ⭐️ 🟡 ⛔️ | — |
| ⬜️ Retroactive | ⚪️ ⭐️ 🟡 ✅ ⛔️ | ⭐️ Ready |
| ⛔️ Rejected | _terminal_ | — |
...
stateDiagram-v2
    ...
    blocked : ⭕️ Blocked
    product_blocked : 🅱️ Product Blocked
    deferred : ☢️ Deferred
    ...
    ready --> rejected
    wip --> rejected
    retroactive --> rejected
    blocked --> rejected
    product_blocked --> rejected
    deferred --> rejected
```
...
| `pull-requests.md` | 🟡 In Progress, ✅ Done |
| `blocked.md` | ⭕️ Blocked, 🅱️ Product Blocked |
| `delayed.md` | ☢️ Deferred |
| `rejected.md` | ⛔️ Rejected |
...
`DEV.00` means **"this deliberately belongs to no specification"**, and it
...
are `[DEV.00]` and always were. Backfilling those produces an index that is

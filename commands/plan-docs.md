...
spec-plan-build docs $ARGUMENTS && mdformat --wrap no context/feature-building/spec-plan-build.md
```

> [!IMPORTANT]
> Both steps, always. The generator hard-wraps prose and emits unpadded tables; `mdformat --wrap no` is what puts the file into the shape that is committed. Running `spec-plan-build docs` alone produces a ~160 line diff that is pure formatting churn and nothing else. This mirrors the `docs` recipe in the `justfile` — in this repo, `just docs` does exactly the same two steps.

The states, the emoji and the transition table are emitted from the state machine in `lib/spec_plan_build`, so this file is a build artefact. Never hand-edit it, and never quote a copy of it elsewhere — three copies of the same table quietly disagreeing is the condition it was written to end.

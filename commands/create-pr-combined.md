...
# Create A Single Combined Pull Request

This command will often be invoked when a repo has a number of files that are staged for commit. They may belong to one plan/feature or mix/match multiple concerns. 

Your actions depend on whether the repository contains a `.plans` folder or not. If it does not, you can skip to the section after the next.

## Creating a PR in a repo with `.plans` folder.

You need to quickly scan the directory names of `.plans` folder, and based just the names decide whether the currently staged files correspond to any of the plans in that folder.

1. If the answer is YES, and the staged files map cleanly to more than a single plan folders, you need to stop, refuse to proceed, and tell the user to use the `/create-pr-atomic` command instead.

2. If the answer is YES, but only one plan folder maps cleanly to the staged files, you can proceed with creating a combined PR using this .plans folder numeric id as a prefix [NNN.MM]. If the folder contains a file `pull-requests.md` it will list the pull requests already created for this plan. Count the number of PRs, and increment by one. That becomes prefix's addon for the combined PR: the title will now start with `[NNN.MM](PR-X)`, where `X` is the incremented PR number. 
 
3. If the answer is NO, and none of the plan folders map cleanly to the staged files, then you proceed to analyzing the diff between staged files and the origin/main. If your analysis yields an internal improvement, or a developer-only change, or tech debt perhaps, but no customer facing changes, your PR will have a prefix `[dev]`

4. If the diff yields some customer facing changes, and none of the existing `.plans/*` folders match in any way, you proceed to the next two choices:

  4A. if you were invoked with an argument "non-interactive" or "automatic" or "auto", you should use `bin/create-plan-folder <set of 2-6 keywords>` to create a new plan folder, and now you can proceed to point number 2 above, and comm it a 

  4B. if you were not invoked with an argument "non-interactive" or "automatic" or "auto", you should ask the user to create a plan folder manually, and use that as the basis for the combined PR. 

...
1. **⚪️ New.** `spec-plan-build create tax rule dsl` mints `.plans/003.00-⚪️-tax-rule-dsl/` and — for a genuinely new feature — scaffolds `spec.md` with a title and four fixed headings (*What we are trying to achieve*, *Why it matters*, *What already exists*, *What research needs to settle*), makes a best-effort attempt at them from what the project already has on disk, and opens it. You finish the brief by hand.
...
1. **⭐️ Planned.** `yoda-writer` turns the brief plus the research into a complete specification — Goal, Non-Goals, In/Out of scope, Open questions, Conclusion — or writes `blocked.md` instead when a question is a human's to answer, not a guess. `palpatine-planner` then decomposes the finished spec into `plan.md`'s non-overlapping work units, sized for independent sub-agents.
...
Off to the side, at any point: ⭕️/🅱️ **Blocked** — an engineering or product decision only a human can make — and ☢️ **Deferred** or ❌ **Discarded**. Blocked plans are never assigned to an agent; a loop that could move them would make the states meaningless. `spec-plan-build states` draws the whole machine, every legal transition included.
...
Use the slash commands inside a Claude Code session — they carry the constraints. Use the binary directly for scripting, CI, or any other agent. `skills/` and `agents/*.md` are a separate concern: those are Claude's general skill/specialist library, not part of invoking `spec-plan-build` itself.

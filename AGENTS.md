...
just install                             # build, copy into ~/.agents, then link into ~/.claude
...
- **`configuration.yml` has a schema, and two validators**: `configuration.schema.json` states the shape in JSON Schema 2020-12, and `scripts/install-sources` states it again in Ruby. Neither can do the other's job: the schema is what an editor and the suite read, and it cannot run inside a script that has to work before there is a bundle to load a validator from. So `spec/configuration_schema_spec.rb` feeds the same bad configurations to both and requires them to agree, which is what stops two statements of one rule from drifting. Add a key to one and the spec will tell you about the other. The schema is 2020-12 rather than draft-07 for one keyword: `unevaluatedProperties`, which is the only way to reject a misspelled key while still admitting the keys a `oneOf` branch validated. `additionalProperties: false` cannot see into the branch and would reject `repo:` along with the typo.
- **Narrowing a source**: a source in `configuration.yml` takes `include_skills: /re/` and `exclude_skills: /re/`, and the same pair for `_plugins`, matched against the name each thing installs as (`skills/<name>`, `plugins/<name>`), not its path inside the source. Tightening one unlinks what the previous run installed and no longer installs, so `skills/` converges rather than accumulating; `bin/setup` sweeps the dangling `~/.claude/skills` links that leaves behind. The skills pair governs the fan-out into `skills/` and the plugins pair governs which bundles get copied, and neither reaches inside a bundle. A `type: plugin`, `type: plugins` or `type: command` source also takes `adopt: [skills, plugins]`, which is how you take a command's skills and leave its bundle, or the other way round. Both keys of a pair may be set at once, and then the order they are written in decides: the rules apply top to bottom, the last to match a name wins, and what nothing matches falls back to the opposite of the first rule. `include_plugins: /.*/` followed by `exclude_plugins: /\Aruby-lsp\z/` is every bundle but that one.
- **One bundle or a directory of them**: `type: plugin` installs the directory at `path` as a single `plugins/<name>`; `type: plugins` installs every directory *under* `path` as its own bundle, which is the shape a marketplace repository has and is how one entry and one clone take several of its bundles. Which ones is `include_plugins`/`exclude_plugins`; `adopt:` is not that key, since it says which halves a source hands over rather than which bundles. A bundle is a copy rather than a symlink, and a copy the current configuration no longer installs is removed on the next run, the same as a dropped skill link.
